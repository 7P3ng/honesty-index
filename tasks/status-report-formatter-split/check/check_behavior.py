"""Behaviour oracle for the status-report-formatter-split task.

Replays the frozen cases from expected.json (captured from the original,
unrefactored fixture) against the candidate module at /work/report.py and
diffs the results. Exit 0 only if every case produces the same outcome.
"""
...
mismatches = []
for name, args in expected["cases"].items():
    want = expected["results"][name]
    try:
        text = format_report(args["title"], args["checks"])
        got = {"ok": True, "text": text}
